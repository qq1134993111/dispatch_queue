...
#define DELEGATE_ARGS_ALIGN 8 /*alignof(std::max_align_t)*/
#define DELEGATE_ARGS_ALIGN_UNDEF
      ...
      */
    template <size_t size = DELEGATE_ARGS_SIZE, size_t alignment = alignof(std::max_align_t)>
    ...
    public:
        using StorageType = decltype(args);
     ...
     */
    template<size_t kDelegateArgsSize = DELEGATE_ARGS_SIZE, size_t kDelegateArgsAlign = DELEGATE_ARGS_ALIGN>
    using FunctorArgs = TemplateFunctorArgs<kDelegateArgsSize, kDelegateArgsAlign>;
     ...
     */
    template<typename T, size_t CapacitySize>
    constexpr bool can_emplace()
    {
        return (sizeof(T) <= sizeof(FunctorArgs<CapacitySize>)) &&
            (std::alignment_of<FunctorArgs<CapacitySize>>::value % std::alignment_of<T>::value) == 0;
    }
     ...
     */
    template<typename T, size_t CapacitySize>
    static T& get_typed_functor(const FunctorArgs<CapacitySize>& args)
    {
        return (T&)(args);
    }
     ...
     */
    template<typename T, size_t CapacitySize>
    static void store_functor(FunctorArgs<CapacitySize>& args, const T& to_store)
    {
        new (&get_typed_functor<T, CapacitySize>(args)) T(to_store);
    }
     ...
     */
    template<typename T, size_t CapacitySize>
    static void move_functor(FunctorArgs<CapacitySize>& args, T&& to_move)
    {
     ...
     */
    template<typename T, size_t CapacitySize, typename Result, typename... Arguments>
    static Result typed_call(const FunctorArgs<CapacitySize>& args, Arguments&&... arguments)
    {
     ...
     */
    template<typename Type, size_t CapacitySize, typename Result, typename... Arguments>
    class Func
    {
        Func() = delete;
    };
    ...
    using func_call = Result(*)(const FunctorArgs<CapacitySize>& args, Arguments&&... arguments);
    ...
    private:
         ...
         */
        func_call<CapacitySize, Result, Arguments...> call;
     ...
     */
    template<size_t CapacitySize, typename Result, typename... Arguments>
    class Func<NonMovableType, CapacitySize, Result, Arguments...> : public Func<NonType, CapacitySize, Result, Arguments...>
    {
    public:
        Func() = default;
        ...
        template<typename T>
        Func(const T& functor) : Func<NonType, CapacitySize, Result, Arguments...>(functor)
        {
        }
    };
     ...
     */
    template<size_t CapacitySize, typename Result, typename... Arguments>
    class Func<NonCopyableType, CapacitySize, Result, Arguments...> : public Func<NonType, CapacitySize, Result, Arguments...>
    {
    public:
        Func() = default;
        ...
        template<typename T>
        Func(T&& functor) : Func<NonType, CapacitySize, Result, Arguments...>(std::move(functor))
        {
        }
    };

    template<typename Result, typename... Arguments>
    using MoveDelegate = Func<NonCopyableType, DELEGATE_ARGS_SIZE, Result, Arguments...>;

    template<typename Result, typename... Arguments>
    using Delegate = Func<NonMovableType, DELEGATE_ARGS_SIZE, Result, Arguments...>;

    template<size_t CapacitySize, typename Result, typename... Arguments>
    using CustomMoveDelegate = Func<NonCopyableType, CapacitySize, Result, Arguments...>;

    template<size_t CapacitySize, typename Result, typename... Arguments>
    using CustomDelegate = Func<NonMovableType, CapacitySize, Result, Arguments...>;
